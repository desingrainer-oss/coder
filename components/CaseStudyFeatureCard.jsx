...
export default function CaseStudyFeatureCard({ study }) {
  ...
  return (
    <div className="bg-white border border-border/80 rounded-2xl shadow-card overflow-hidden flex h-full min-h-[300px] card-lift">
      <div className="relative w-[42%] shrink-0 my-5 ml-5 rounded-xl overflow-hidden self-stretch min-h-[240px]">
        <Image
          src={cover}
          alt={study.client}
          fill
      ...
      <div className="flex flex-col flex-1 py-6 pr-6 pl-5 min-w-0">
        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.12em] mb-2.5">
          {study.tag}
        </span>
        <h3 className="text-[17px] font-bold text-navy leading-[1.45] mb-5 pr-2">{study.title}</h3>

        <ul className="flex flex-col gap-2.5 flex-1">
          {study.metrics.map((m) => (
            <li key={m.l} className="text-[13.5px] leading-snug">
        ...
        <Link
          ...
          className="link-blue text-[13.5px] font-semibold self-end mt-5 shrink-0"
        >
          Ver caso completo →
        </Link>
      </div>
