...
function StepCard({ step }) {
  return (
    <div className="flex items-center gap-3">
      <div className="w-14 h-14 sm:w-[72px] sm:h-[72px] rounded-full bg-[#eef1f6] flex items-center justify-center shrink-0">
        {step.icon}
      </div>
      <div className="min-w-0">
        <div className="text-[20px] sm:text-[22px] font-extrabold text-navy leading-none mb-1">{step.num}</div>
        <p className="text-[13px] font-semibold text-navy leading-snug max-w-none sm:max-w-[108px]">{step.label}</p>
      </div>
    </div>
  );
}
...
export default function Methodology() {
  return (
    <div id="metodologia" className="py-12 md:py-16 lg:py-[72px] px-4 sm:px-6 md:px-10 bg-white">
      <AnimateIn>
        <p className="text-[11px] font-bold tracking-[1.5px] uppercase text-blue mb-2.5 text-center">
          ASÍ TRABAJAMOS
        </p>
        <h2 className="text-[24px] sm:text-[28px] md:text-[32px] font-extrabold text-navy mb-8 md:mb-14 tracking-tight text-center">
          Una metodología comprobada para lograr resultados
        </h2>
      </AnimateIn>

      <div className="max-w-[1180px] mx-auto">
        <Stagger className="flex flex-col gap-5 max-w-md mx-auto lg:hidden" stagger={0.12}>
          {steps.map((step) => (
